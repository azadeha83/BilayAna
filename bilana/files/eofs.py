...
class EofScd(Neighbors):
    ...
    def assemble_eofs(self, energyfile, timetoscd, timetoorientation, lipidpair, endtime):
        ...
        if 'CHL1' in self.molecules:
            ...
            components += ['Facing'] # Ergs ... not to both
            components += ['Notfacing'] # Ergs ... not to both
            components += ['Orient_flag'] # Ergs ... not to both
        ...
        with open(energyfile, "r") as efile, open(outname, "w") as outf:
            ...
            print(\
                  ...
                  .format("Time", "Host", "Host_Scd", "Neib", "Neib_Scd",
                            ...
                            "Etot", "Evdw",
                            "Ecoul", "Ntot")\
                            + (len(components)*'{: <15}').format(*components), file=outf)
            ...
            for line in efile:
                ...
                print(
                      ...
                      .format(time, host, scd_host, neib, scd_neib,
                              ...
                              float(COUL), ntot)\
                      + (len(neib_comp_list)*'{: <15}').format(*neib_comp_list) + (len(sterol_comp_list)*'{: <15}').format(*sterol_comp_list),file=outf)
