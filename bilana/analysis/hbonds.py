...
class Hbonds(SysInfo):
    ...
    def h_bonds_sterol_lipid(self,end_time):
        ''' This module calculates the hbonds between sterol and lipid molecules '''

        cwd = os.getcwd()

        index_hbond = ''.join(cwd + '/' + 'index_hbond_.ndx')
       
        hbond_raw = ''.join(cwd + '/' + 'hbond_sterol_lipid_raw')

        get_selection = [GMXNAME, 'select', '-f', self.trjpath, '-s', self.tprpath, '-on', index_hbond, \
            '-select', '(resname {})'.format(self.lipid_types_sterol)]
        inp_str = b'DPPC\n{}\n0\n'.format(self.lipid_types_sterol)
        #print(get_selection)
        
        out, err = exec_gromacs(get_selection, inp_str)

        get_hbond = [GMXNAME, 'hbond', '-f', self.trjpath, '-s', self.tprpath, '-n', index_hbond, \
            '-num', hbond_raw]
            
        #print(get_msd)  
        out, err = exec_gromacs(get_hbond)

        with open("hbond_sterol_lipid_raw", 'r') as f:
            ls = f.readlines()
    ...
    # def h_bonds_sterol_sterol(self,end_time):
