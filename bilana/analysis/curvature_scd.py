...
class Curvature(SysInfo):
    ...
    def __init__(self,bin_width,inputfilename="inputfile"):
        ...
        self.lipid_head_up = self.u.select_atoms('resname {} and name P O11 O12 O13 O14 and (prop z > {})'.format(self.main_lipid,main_lipid_selection_center[2])) 
        self.lipid_head_down = self.u.select_atoms('resname {} and name P O11 O12 O13 O14 and (prop z < {})'.format(self.main_lipid,main_lipid_selection_center[2]))
        ...
        self.x = self.x_edges[:]
        self.bins = int((self.x_max-self.x_min)/self.bin_width)
    ...
    def scd_calculation(self,start_time,end_time):
        ...
        for i_ts,ts in enumerate(self.u.trajectory[start_time:end_time:]):
            ...
            scd_data_bin = np.empty(len(self.x_edges)-1)

            for i_x in range(self.bins):

                x_slice_min = self.x_edges[i_x]
                x_slice_max = self.x_edges[i_x+1]
                lipid_sel = self.u.select_atoms('resname {} and name P and (prop x > {}) and (prop x < {})'.format(self.main_lipid,x_slice_min,x_slice_max))              

                if len(lipid_sel) != 0:
                   
                    resids_list = list(lipid_sel.resids)
                    
                    scd_i = 0
                    for res in resids_list:
                        scd_value = calc_s(self.u, res)
                        scd_i += scd_value
                    scd_mean = scd_i / len(resids_list)
                    scd_data_bin[i_x] = scd_mean
                else:
                    scd_data_bin[i_x] = 0
            scd_data.append(scd_data_bin)
        scd_data_mean = np.mean(scd_data, axis=0)
        self.scd.append(scd_data_mean)       


    def run_calculations(self,end,step):
        ...
        pp = PdfPages('z_profile_fourier_scd.pdf')
        ...
        for i,j in enumerate(m):
            ...
            ax.plot(self.x_edges[:-1], m[i,:], label = 'frame {}'.format(i))
        ...
        for i,j in enumerate(m):
            ...
            fit = np.polyfit(a[i,:-1],m[i,:],1)
            fit_fn = np.poly1d(fit)
            color = next(ax._get_lines.prop_cycler)['color']
            ax.plot(a[i,:-1], m[i,:], 'o', a[i,:-1], fit_fn(a[i,:-1]),'-', markeredgecolor='none', label = 'frame {}'.format(i), color=color)
            ax.plot(a[i,:-1], fit_fn(a[i,:-1]),'-', color=color)
