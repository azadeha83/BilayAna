...
class Cholesterol_inputplots():
    def __init__(self, systemnames=Tranges.keys(), averaging=True):
    ...
    def plot_N_chol(self, systemname, filename, averaging=True):
        ...
        plot = ggplot2.ggplot(tavg)\
            + aes\
            + ggplot2.scale_x_continuous('N cholesterol neighbors')\
...
class PofScdplots:
    def __init__(self, systemnames=Tranges.keys()):
        ...
        for syst in systemnames:
            self.Tlow[syst], self.Thigh[syst] = Tranges[syst]
    def plot_standard(self, systemname, lipid, filename):
        ''' '''
        tavg = self.read_data(systemname, systemname, lipid)
        ...
        plot = ggplot2.ggplot(tavg)\
            ...
            + ggplot2.scale_y_continuous(name='expression("S"[CD])')\
            + ggplot2.scale_size()\
            + guide\
            + ggplot2.theme_light() + my_theme()
        gr.pdf(file=filename, width=10, height=7)
        ...
        gr.dev_off()
        ro.r('rm(list=ls(all=TRUE))')
    def read_data(self, outputtable, systemname, lipid):
...
class NofScdplots():
    ...
    def plot_standard(self, systemname, lipid, filename, y_type='Ntot', neib_spec=None, averaging=True, plotrel=False):
        ...
        if averaging == False and neib_spec is not None:
            plot += ggplot2.scale_shape_manual(values=ro.r("1:"+str(len([i for i in range(self.Tlow[systemname], self.Thigh[systemname]+1, 10)]))))
        gr.pdf(file=filename, width=10, height=7)
        print(plot)
        gr.dev_off()
        ro.r('rm(list=ls(all=TRUE))')
...
class Selfinteraction():
    ...
    '''
    def __init__(self, interaction, systemnames=Tranges.keys(), averaging=True):
...
class EofScdplots():
    ...
    '''
    def __init__(self, interaction, systemnames=Tranges.keys(), averaging=True):
        pairs = [('DPPC_DPPC', (-90, -50, 5)), ('DUPC_DUPC', (-90, -50, 5)), ('DPPC_DUPC', (-90, -50, 5)), ('DPPC_CHL1', (-50, -20, 5)), ('DUPC_CHL1', (-50, -20, 5)), ('CHL1_CHL1', (-30, -10, 5))]
        self.enthalpyranges = {}
        self.systemnames = systemnames
        self.Tlow = {}
        self.Thigh = {}
    ...
    def plot_H_lipidchol(self, systemname, pair, filename, l_orderparam, mdpath, averaging=True, neib_spec=None):
...
#'\n{0}fin$weight <- {0}fin$weight/sum({0}fin$weight)'
        ...
        if neib_spec is None:
            ...
            plot = ggplot2.ggplot(final_table)\
                    + ggplot2.aes_string(x='AvgScd', y='Etot', shape='factor(System)', color='factor('+neib_spec+')', weight='weight')\
                    + add_axis('x', 'avgscd', breaks=[-0.2, 1.0, 0.2], pair=pair)\
                    ...
                    + ggplot2.scale_y_continuous(name= ro.r('expression(paste("H","(S"[CD],") / kJ/mol"))'))\
                    + ggplot2.guides(color=ggplot2.guide_legend(neib_spec), shape=ggplot2.guide_legend("System"), linetype=ggplot2.guide_legend("System"),size=False)\
                    + ggplot2.scale_size("weight")\
                    + ggplot2.ggtitle(title+' interaction')\
                    + ggplot2.theme_light() + my_theme() + ggplot2.theme(**{'legend.key.width':ro.r.unit(1,"cm")})
                    #===========================================================
...
#                + ggplot2.geom_smooth(se='False')\
    ...
    def standard_plot(self, systemname, pair, interaction_pair, filename, averaging=True, neib_spec=None):
        ...
        if neib_spec is None:
            ...
            yaxis = add_axis('y', 'enthalpy', breaks=self.enthalpyranges[pair], pair=pair)
            guides = ggplot2.guides(color=ggplot2.guide_legend("T / K"), linetype=ggplot2.guide_legend("T / K"))
        elif neib_spec == 'Ntot':
            if averaging:
            ...
            xaxis = add_axis('x', 'avgscd', breaks=[-0.2, 1.0, 0.2], pair=pair)
            yaxis = ggplot2.scale_y_continuous(name= ro.r('expression(paste("H","(S"[CD],") / kJ/mol"))'))
            guides = ggplot2.guides(color=ggplot2.guide_legend(neib_spec))
            #===========================================================
            # + add_axis('y', 'enthalpy', breaks=self.enthalpyranges[pair], pair=pair)\
            #===========================================================
        else:
            ...
            xaxis = add_axis('x', 'avgscd', breaks=[-0.2, 1.0, 0.2], pair=pair)
            yaxis = add_axis('y', 'enthalpy', breaks=self.enthalpyranges[pair], pair=pair)
            guides = ggplot2.guides(color=ggplot2.guide_legend(neib_spec), shape=ggplot2.guide_legend("T / K"), linetype=ggplot2.guide_legend("T / K"))
        print(aes, )
        plot = ggplot2.ggplot(tavg)\
                + aes\
                + guides\
        ...
        if not averaging:
            ...
            if neib_spec is not None:
                df_final = ro.r('\n{0}Tlow <- {0}[temperature<325][,temperature:=NULL]'
                                ...
                                '\n{0}Tlow <- {0}Tlow[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction, {1})]'
                                '\n{0}Thigh <- {0}Thigh[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction, {1})]'
                                '\n{0}compl <- {0}compl[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction, {1})]'
                ...
                df_final = ro.r('\n{0}Tlow <- {0}[temperature<325][,temperature:=NULL]'
                                '\n{0}Thigh <- {0}[temperature>325][,temperature:=NULL]'
                                '\n{0}compl <- {0}[,temperature:=NULL]'
                                '\n{0}Tlow <- {0}Tlow[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction)]'
                                '\n{0}Thigh <- {0}Thigh[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction)]'
                                '\n{0}compl <- {0}compl[,lapply(.SD,weighted.mean,w=weight),by=list(binAvgScd, Interaction)]'
                                '\n{0}Tlow$temperature <- "290-320 K"'
                                '\n{0}Thigh$temperature <- "330-350 K"'
                                '\n{0}compl$temperature <- "complete"'
                                '\ninsignificantlow <- 0.015*max({0}Tlow$weight)'
        ...
        if interaction_pair == 'complete':
            if neib_spec == 'Ntot':
                ...
                df_final = ro.r('{0} <- subset({0},select=-c(temperature))'
                                ...
                                '\ninsignificant <- 0.01*max({0}$weight)'
                                '\n{0} <- {0}[weight>insignificant][Ntot<8][3<Ntot]'
                                '\n{0}'
                                .format(outputtable, neib_spec))#column, ))
            elif neib_spec is None:
                df_final = ro.r('{0} <- subset({0},select=-c(temperature))'
